                    ...
                    buttons: {
                        "Ok": function ()
                        {
                            ...
                            if (selected.length > 0) {
                                ...
                                {
                                    selectForValidation.on('boxend', function (event) {
                                        var featuresForValidation = [];
                                        var extent = selectForValidation.getGeometry().getExtent();
                                        map.getLayers().forEach(function (layer) {
                                            if (layer instanceof ol.layer.Vector && layer.get('name') === 'Mast:la_spatialunit_land') {
                                                layer.getSource().forEachFeatureIntersectingExtent(extent, function (feature) {
                                                    featuresForValidation.push(feature);
                                                });
                                            }
                                        });
                                        getIntersections(featuresForValidation);
                                    });

                                    var checked = $("#" + e.currentTarget.id).hasClass('ui-state-active1')
                                    if (checked) {
                                        map.addInteraction(selectForValidation);
                                    }
                                    ...
                                    map.getLayers().forEach(function (layer) {
                                        if (layer instanceof ol.layer.Vector && layer.get('name') === 'Mast:la_spatialunit_land') {
                                            getIntersections(layer.getSource().getFeatures());
                                        }
                                    });
                                    spatialDialog.dialog("destroy");
                                }
