...
public class WorkflowActionHibernateDAO extends GenericHibernateDAO<WorkflowActionmapping, Long> implements WorkflowActionDAO {
    ...
    public List<WorkflowActionmapping> getWorkflowActionmapping(Integer workflowid, Integer roleid) {
        ...
        String queryString = "select w from WorkflowActionmapping w where w.isactive= true and w.workflow.workflowid = :workflowid and w.roleid =:roleid order by w.worder";

        try {
            lstWorkflowActionmapping = getEntityManager().createQuery(queryString).setParameter("workflowid", workflowid).setParameter("roleid", roleid).getResultList();
        } catch (Exception e) {
