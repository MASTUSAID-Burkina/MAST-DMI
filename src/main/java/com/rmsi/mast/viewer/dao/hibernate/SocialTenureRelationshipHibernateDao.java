...
public class SocialTenureRelationshipHibernateDao extends GenericHibernateDAO<SocialTenureRelationship, Long>
    ...
    public boolean updateSocialTenureRelationshipByPartytypeId(Long partyId, Long landid,
            int mutationTypeId, String contractName, String contractNumber, Date contractDate, String apfrNum, int ownershipTypeId) {
        try {
            Query query = getEntityManager().createQuery("UPDATE SocialTenureRelationship st "
                    + "SET st.laPartygroupPersontype.persontypeid=1, st.mutationId = :mutationId, st.shareTypeId = :shareTypeId,"
                    + "st.transferContractName = :contractName, st.transferContractNum = :contractNumber, "
                    + "st.transferContractDate = :contractDate, st.certNumber = :apfrNum, st.certIssueDate = :today "
                    + "where st.partyid = :partyid and st.landid = :landid");
            query.setParameter("partyid", partyId)
                    .setParameter("landid", landid)
                    .setParameter("mutationId", mutationTypeId)
                    .setParameter("shareTypeId", ownershipTypeId)
                    .setParameter("contractName", contractName)
                    .setParameter("contractNumber", contractNumber)
                    .setParameter("contractDate", contractDate)
                    .setParameter("apfrNum", apfrNum)
    ...
    public SocialTenureRelationship getSocialTenureRelationshipByTransactionId(
        ...
        try {
            Query query = getEntityManager().createQuery("Select su from SocialTenureRelationship su where (su.laExtTransactiondetail.transactionid = :landid and su.isactive=true)");
            List<SocialTenureRelationship> lstSocialTenureRelationship = query.setParameter("landid", transid.intValue()).getResultList();
    ...
    public SocialTenureRelationship getAllSocialTenureRelationshipByTransactionId(
        ...
        try {
            Query query = getEntityManager().createQuery("Select su from SocialTenureRelationship su where (su.laExtTransactiondetail.transactionid = :landid)");
            List<SocialTenureRelationship> lstSocialTenureRelationship = query.setParameter("landid", transid.intValue()).getResultList();
